    ...
    async requestPayment(memo, amount, callbacks) {
        ...
        try {
            const payment = await Pi.createPayment({
                amount: amount,
                memo: memo,
                metadata: { type: 'tetris_revive' },
            }, {
                onReadyForServerApproval: (paymentId) => {
                    console.log('Payment ready for approval', paymentId);
                    // Usually you send this to your backend, then backend approves it.
                    // For client-only test, Pi sandbox auto-approves if configured, or you must handle backend.
                ...
                onReadyForServerCompletion: (paymentId, txid) => {
                    ...
                    // Send txid to backend to complete.
